...
* **Pengguna:** Camat, Sekretaris Kecamatan (Sekcam), atau Tim Koordinasi Bansos Kecamatan.
* **Tujuan & Tanggung Jawab:**
  - Pemegang otoritas penetapan keputusan akhir (*final approval*) penerima bansos se-kecamatan.
  - Mengelola alokasi kuota dan distribusi anggaran bansos antar-kelurahan.
  - Menindaklanjuti sanggahan masyarakat yang masuk dari kelurahan.
* **Halaman yang Dapat Diakses:**
  - `/approval` — Tahap 4 / Final Approval: Menerbitkan penetapan resmi (`disetujui_kecamatan`) atau `ditolak`.
  - `/dashboard` — Dashboard eksekutif kecamatan (anggaran terserap, rekapitulasi kelurahan, tren pengentasan kemiskinan).
  - `/analitik` — Analisis mendalam anomali penerima bansos ganda dan distribusi program.

---

### 👑 6. `super_admin` (Dinas Sosial / Administrator Utama)
* **Pengguna:** Tim IT Dinas Sosial Kabupaten/Kota atau Administrator Sistem Tingkat Tinggi.
* **Tujuan & Tanggung Jawab:**
  - Manajemen penuh sistem bansos lintas seluruh wilayah (kecamatan, kelurahan, RW, RT).
  - Pengawasan forensik data dan pencegahan korupsi/kecurangan bansos.
  - Konfigurasi parameter scoring kelayakan (PMT & AI Weighting) dan master program bansos.
* **Halaman yang Dapat Diakses:**
  - Seluruh modul petugas dan masyarakat tanpa batasan wilayah (*all access*).
  - Dapat melakukan intervensi approval pada semua tahapan (Bypass approval).
  - `/audit-log` — Log audit forensik lengkap mencatat seluruh aksi, nama petugas, waktu, status perubahan, IP, dan alasan keputusan secara tidak dapat diubah (*tamper-proof*).

---

## 3. Alur Workflow Persetujuan Berjenjang (Multi-Tier Approval)

Berikut alur kerja penetapan bantuan sosial dari survei lapangan hingga penetapan:

```mermaid
flowchart TD
    Start([Mulai: Warga Membutuhkan]) --> S1[1. Petugas RT Melakukan Survei Lapangan & Foto Rumah]
    S1 --> PMT[Sistem AI & PMT Menghitung Skor Kelayakan & Desil]
    PMT --> A1{Petugas RT Usulkan?}
    
    A1 -- Ya --> St1[Status: diusulkan_rt]
    A1 -- Tidak --> EndDitolak([Tidak Diusulkan])
    
    St1 --> A2{2. Musyawarah RW\nPetugas RW}
    A2 -- Setuju --> St2[Status: disetujui_rw]
    A2 -- Tolak / Revisi --> RejRW[Status: ditolak / dikembalikan]
    
    St2 --> A3{3. Verifikasi Kelurahan\nPadan DTKS & Kuota}
    A3 -- Valid --> St3[Status: diverifikasi_kelurahan]
    A3 -- Tidak Valid --> RejKel[Status: ditolak]
    
    St3 --> A4{4. Penetapan Kecamatan\nPetugas Kecamatan}
    A4 -- Ditetapkan --> StFinal[Status: disetujui_kecamatan\nBansos Diterbitkan]
    A4 -- Ditolak --> RejKec[Status: ditolak]
    
    StFinal --> WargaView[Warga Melihat Status di /profil]
    RejKec --> SanggahFlow[Warga Mengajukan /sanggahan via AI]
```
